<template>
  <div>
    <!-- <button class="btn btn-primary" :disabled="loading" @click="showModal = true" style="background-color: #2FB490 !important; border-radius: 9px !important;">
      ...
      <div class="modal-dialog" role="dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <h5>Deseja editar o cliente: {{user.name}} ?</h5>
          </div>
          <div class="modal-footer">
            <button class="btn btn-desactivate" @click="closeModal()">
              Cancelar
            </button>
            <button v-if="process === 'initial'" class="btn btn-activate" @click="EditCustomer()">
              <template v-if="loading" style="transition: all .3s !important">
                <i class="fas fa-circle-notch fa-spin"></i>
              </template>
              <template v-else>
    ...
    async EditCustomer () {
      ...
      this.$firebase.database().ref('users').child(this.user.uId).update(this.user, err => {
        if (err) {
          ...
          this.$toast("Cliente Editado com sucesso", {
          timeout: 2000
      });
        }
      })
