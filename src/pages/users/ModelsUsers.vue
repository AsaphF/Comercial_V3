              ...
              <div v-if="id === 'email'" class="container-list-email">
                ...
                <div class="models-container">
                  <template v-for="(m, index) in listEmail">
                    <div v-if="m.id === model.id" class="model-container activated" :key="index"  @click="selectModel(m)">
                    ...
                    <div v-else class="model-container" :key="index" @click="selectModel(m)">
                      <div class="template-type">
                        <span>Template de {{ m.type }}</span>
                        <svg v-if="m.favorite && m.favorite === true" width="21" height="21" viewBox="0 0 24 24" fill="#F2C335" xmlns="http://www.w3.org/2000/svg">
                          <path fill-rule="evenodd" clip-rule="evenodd" d="M12 2.896L14.935 8.889L21.5 9.856L16.75 14.518L17.871 21.104L12 17.993L6.129 21.104L7.25 14.518L2.5 9.856L9.064 8.889L12 2.896Z" stroke="#e6ac00" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                ...
                <div class="buttons-container">
                  ...
                  <button v-else class="btn btn-sm btn-send-mail" disabled>
              ...
              <div v-else class="container-list">
                ...
                <div class="models-container">
                  <template v-for="(m, index) in listWhats">
                    <div v-if="m.id === model.id" class="model-container activated" @click="selectModel(m)" :key="index">
                    ...
                    <div v-else class="model-container" @click="selectModel(m)" :key="index">
                      <div class="first-line-container">
                        <h6 v-if="m.title.length > 80"><strong>Título do Modelo:</strong> {{ m.title.slice(0, 80) }}...</h6>
                        <h6 v-else><strong>Título do Modelo:</strong> {{ m.title }}</h6>
                        <template>
                ...
                <div class="buttons-container">
                  ...
                  <button v-else-if="process === 'loading'" class="btn btn-send-whats">
                    Carregando
                    <svg class="loading-clock" version="1.1" id="L2" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                      viewBox="0 0 100 100" enable-background="new 0 0 100 100" xml:space="preserve">
                      <circle fill="none" stroke="#fff" stroke-width="4" stroke-miterlimit="10" cx="50" cy="50" r="48"/>
...
export default {
  ...
  methods: {
    openModal (id) {
      this.showModal = true
      if (!this.affiliate.botConversaAPI && id === 'botConversa') {
      ...
      if (this.id === 'botConversa') {
        if (this.affiliate.botConversaAPI) {
          ...
          if (selectedModel.length > 0) {
            ...
            message = this.checkMessage(message, this.user)
            const fomartedPhone = this.formatedPhone(this.user.phone)

            // Envio dos dados através da API

            if (this.user.botConversaId) {
              const id = this.user.botConversaId
              ...
              if (response.status === 200) {
                ...
                await this.createAndSend(this.user, firstName, lastName, fomartedPhone, message, title)
              } else {
                this.process = 'error'
              ...
              this.createAndSend(this.user, firstName, lastName, fomartedPhone, message)
            }
          } else {
        ...
        if (selectedModel.length) {
          let message = selectedModel[0].text
          message = this.checkMessage(message, this.user)
          const phoneNumber = this.user.phone.replace(/[^a-zA-Z0-9]/g, '')
          tempObj.modelTitle = selectedModel[0].title
          await this.$firebase.database().ref(`support/messageHistoric/${window.uid}`).child(`whatsMessage/${Date.now()}`).update(tempObj)
          window.open(`https://wa.me//55${phoneNumber}?text=${message}`, '_blank')
        } else {
          this.status = {
            info: 'É necessário selecionar um modelo',
            color: 'alert-warning'
          }
          setTimeout(() => {
            this.status = {
    ...
    async createAndSend (user, firstName, lastName, fomartedPhone, message, title) {
      ...
      if (result.status !== 200) {
        ...
        await this.$firebase.database().ref(`users/${this.this.user.uId}`).update({ botConversaId: id })
      ...
      if (selectedModel.length > 0) {
        try {
          ...
          if (this.model.type === 'Apresentação') {
            ...
            const result = await sendSalesBusiness(this.affiliate, this.user.name, this.user.email, this.mailTitle, this.mailBody)
            if (result === 200) {
              const date = new Date()
              await this.$firebase.database().ref(`users/${this.user.uId}`).update({ lastSendDate: date })
              await this.$firebase.database().ref(`support/messageHistoric/${window.uid}`).child(`mailMessage/${tempObj.date}`).update(tempObj)
            } else {
              console.log('Essa usuário não deu certo:' + this.user)
              this.process = 'error'
            }
          } else {
            const tempObj = {
            ...
            const result = await sendSalesMessage(this.affiliate, this.user.name, this.user.email, this.mailTitle, this.mailBody)
            if (result === 200) {
              const date = new Date()
              await this.$firebase.database().ref(`users/${this.user.uId}`).update({ lastSendDate: date })
              await this.$firebase.database().ref(`support/messageHistoric/${window.uid}`).child(`mailMessage/${tempObj.date}`).update(tempObj)
              ...
              console.log('Esse usuário não deu certo:' + this.user)
              this.process = 'error'
            }
          }
          this.process = 'concluded'
    ...
    checkMessage (message, user) {
      ...
      for (const word of wordsInTitle) {
        if (word.toLowerCase() === 'nome') {
          ...
          const letter = this.user.name[0].toUpperCase()
          const name = this.user.name.toLowerCase().slice(1)
