<template>
  <div>
    <div class="button-container">
      ...
      <button class="btn-action"  @click="showModal = true" :disabled="user.actived"><i class="fas fa-play"></i></button>
    </div>
      ...
      <div class="modal-dialog" role="dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <h5>Deseja ativar o cliente para teste: {{user.name}} ?</h5>
          </div>
          <div class="modal-footer">
            <button class="btn btn-desactivate" @click="closeModal()">
              Cancelar
            </button>
            <button v-if="process === 'initial'" class="btn btn-activate" @click="activeUser()">
            ...
            <button v-else-if="process === 'concluded'" class="btn btn-activate">
              Concluído
...
export default {
  ...
  methods: {
    ...
    activeUser () {
      ...
      this.$firebase.database().ref('users').child(this.user.uId).update({ actived: true }, err => {
        if (err) {
          console.log(err)
        }
        // else {
        //   this.status = {
        //     info: 'Cliente Ativado',
        //     color: 'alert-success'
        //   }
        // }
      })
      ...
      setTimeout(() => {
        this.$toast("Cliente Ativado com sucesso!", {
          timeout: 2000
        })
