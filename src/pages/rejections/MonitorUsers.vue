<template>
    <div>
      <button class="btn  btn-sm btn-primary-border" style="width: 140px; padding: 8px 20px !important; margin-right: -10px; margin-left: -5px;" @click="initialChanges()">Monitorar</button>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                  <div class="lead-info-container">
                    <div class="name-container">
                      <small>Nome</small>
                      <p class="lead-name"> {{ user.name.slice(0, 27).toUpperCase() }}</p>
                    </div>
                    ...
                    <div class="row origin-container">
                      <div class="col-6 left-container">
                        ...
                        <p v-if="user.pagarmePlan" class="lead-origin"> {{ user.pagarmePlan.name }}</p>
                      ...
                      <div class="col-6 right-container">
                        ...
                        <p v-if="user.pagarmePaymentStatus" class="lead-campaign"> {{ user.pagarmePaymentStatus }}</p>
              ...
              <div class="buttons-section">
                ...
                <div class="inner-container">
                  <SendProposal :user="user" :loggedAffiliate="loggedAffiliate" @checkSend="handleCheckSend"/>
              ...
              <div class="buttons-section">
                ...
                <div class="inner-container">
                  ...
                  <edit-user :userData="user" @update="handleUpdateUser"/>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    async initialChanges () {
      ...
      if (this.user.status && this.user.status.info) {
        this.statusValue = this.user.status.info
        this.statusCopy = this.user.status.info
      }
      if (this.user.status && this.user.status.dtCallback) {
        this.dtReturn = moment(this.user.status.dtCallback).format('YYYY-MM-DD')
        this.dtReturnCopy = moment(this.user.status.dtCallback).format('YYYY-MM-DD')
      }
      if (this.user.status && this.user.status.obs) {
        this.obs = this.user.status.obs
        this.obsCopy = this.user.status.obs
      }
      this.loading = false
    },
    async saveChanges () {
      ...
      this.user.status = responseObj
      await this.$firebase.database().ref(`support/monitoringComercial/${this.user.uId}`).update(responseObj)
      // Get toast interface
