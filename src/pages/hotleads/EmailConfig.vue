                ...
                <div v-else class="creation-container">
                  <div v-if="!templateList" class="container-template">
                    ...
                    <div class="container-template-footer">
                      <button v-if="modelEmailList.length > 0" class="btn-undo" @click="getBack()">
                          ...
                          <div class="inner-form-container">
                            <div class="help-container">
                              <div class="container-title">
                                ...
                                <input v-model="firstTemplate.templateTitle" class="input-title" type="text" name="" id="" placeholder="Título do modelo do texto para WhatsApp" maxlength="80" required>
                              </div>
                              <div class="buttons-container">
                              ...
                              <div class="inner-container-report">
                                <div class="mail-container">
                                  ...
                                  <div class="main-container">
                                    ...
                                    <textarea v-else v-model="textTitleCopy" class="title-textarea" placeholder="Saudação do E-mail" :disabled="!showAndHide"  maxlength="35" required></textarea>
                                    ...
                                    <textarea v-else v-model="textBodyCopy" class="body-textarea"  placeholder="Corpo do E-mail" :disabled="!showAndHide" maxlength="596" required></textarea>
                      ...
                      <form @submit.prevent="saveAndSend()">
                        <div class="inner-second-container">
                          <div class="help-container">
                            <div class="container-title">
                              <label class="label-title" for="">Modelo:</label>
                              <input v-model="secondTemplate.templateTitle" class="input-title" type="text" name="" id="" placeholder="Título do modelo do texto para WhatsApp" maxlength="80" required>
                            </div>
                            <div class="buttons-container">
                              <div class="eye-icon-container" @click.stop="checkMessage(secondTemplate.textAreaMailTitle, secondTemplate.textAreaMailBody)">
                                <template v-if="showAndHide">
...
export default {
  ...
  computed: {
    ...
    list () {
      const tempUsers = this.modelEmailList
      let favoritesList = []
      let unfavoritesList = []
      let notHave = []
      let result = []
  ...
  methods: {
    async getInfo () {
      this.modelEmailList = this.modelsLists
    ...
    toFavorite (model) {
      if (model.favorite) {
        ...
        this.$firebase.database().ref(`textsModel/${window.uid}`).child(`mailMessage/${model.id}`).update({ favorite: false })
      } else {
        model.favorite = true
        this.$firebase.database().ref(`textsModel/${window.uid}`).child(`mailMessage/${model.id}`).update({ favorite: true })
      }
    },
    editModel (model) {
      this.template = {}
    ...
    handleExclusion (model) {
      this.modelEmailList = this.modelEmailList.filter((m) => m.id !== model.id)
      if (this.modelEmailList.length === 0) {
        this.listSaved = false
        this.initial = true
      }
    ...
    async saveAndSend () {
      ...
      if (this.oneOrOther === true) {
        if (this.template.id) {
          ...
          this.modelEmailList.splice(this.modelEmailList.indexOf(this.template), 1, tempObj)
          // this.modelEmailList.push(tempObj)
