        ...
        <div class="modal-content">
          <div class="modal-header">
            <div class="upward-header">
              <h4 style="font-weight: 700; margin-top: 10px; margin-left: 5px;">Configuração do WhatsApp</h4>
                              ...
                              <div class="modal-body">
                                <div class="selection-container">
                                    <div class="inner-first-container">
                                      <p>Ao criar um modelo de texto, será possível usá-lo para o envio via WhatsApp.</p>
                                    ...
                                    <div class="container-title">
                                      ...
                                      <input v-model="modelTitleWhats" class="input-title" type="text" name="" id="" placeholder="Título do modelo do texto para WhatsApp" maxlength="80" required>
                                    </div>
                                ...
                                <div class="report-container">
                                    <div class="inner-container-report">
                                      <textarea v-if="showAndHide" v-model="textAreaWhats" style="font-size: 14px;"  placeholder="Caso queira, use as informações dos clientes fornecidas na tabela para o seu contato."  required></textarea>
                                      <textarea v-else v-model="textAreaCopy" style="font-size: 14px;"  placeholder="Caso queira, use as informações dos clientes fornecidas na tabela para o seu contato." :disabled="!showAndHide" required></textarea>
                                    </div>
                                </div>
                              </div>
                ...
                <div v-else class="container-list">
                  <div class="inner-container">
                    ...
                    <div v-if="warningAlert" class="info-status status-warning warning">
                      <i style="margin-right: 8px; color: var(--warning);" class="fas fa-exclamation-triangle"></i>Modelo de texto excluido com sucesso
                    </div>
...
export default {
  props: ['lead', 'affiliate', 'modelsLists'],
  components: {
    Historic,
    DeleteModel
  },
  ...
  computed: {
    ...
    list () {
      const tempUsers = this.modelsLists
      let favoritesList = []
      let unfavoritesList = []
      let notHave = []
      let result = []
  ...
  methods: {
    async openModal () {
      this.modelsList = this.modelsLists
      ...
      this.$firebase.database().ref(`support/messageHistoric/${window.uid}/whatsMessage`).once('value', snapshot => {
        ...
        if (values) {
          ...
          this.historic = this.historic.filter(h => h.origin === 'leads')
        }
      })
    },
    switchTabs () {
    ...
    excludeModel (model) {
      this.modelsList = this.modelsList.splice(this.modelsList.indexOf(model), this.modelsList.indexOf(model))
      this.$firebase.database().ref(`textsModel/${window.uid}`).child(`whatsMessage/${model.id}`).remove()
    ...
    handleList (model) {
      this.modelsLists = this.modelsList.filter((mod) => mod.id !== model.id)
      if (this.modelsLists.length === 0) {
        this.edit = false
        this.initial = true
      }
      this.$toast('Modelo do texto Excluído com sucesso', {
        timeout: 2000,
      })
      setTimeout(() => {
        this.warningAlert = false
    ...
    .body-content {
      ...
      .container-list {
        ...
        .models-container{
          ...
          padding: 10px 30px 25px 45px !important;
